...
export default function App(){
  ...
  function arrange(){
    ...
    const already=new Set(nextGroups.flatMap(g=>g.students.map(s=>s.id)))
    students.forEach(s=>{ if(!already.has(s.id)) pool.push(s) })
    ...
    const canPut=(i:number)=>nextGroups[i].students.length<targets[i]
    const put=(i:number, st:Student)=>{ const g=nextGroups[i]; if(g.students.length>=targets[i]) return false; if(!canAddTo(g,st)) return false; nextGroups[i]={...g,students:[...g.students,st]}; return true }

    if(mode==='남여섞기OFF'){
      const male:number[]=[], female:number[]=[], empty:number[]=[]
      nextGroups.forEach((g,i)=>{ const gg=groupGenderOf(g); if(gg==='남') male.push(i); else if(gg==='여') female.push(i); else if(gg==='비어있음') empty.push(i) })
      const baseMale=male.slice()
      const needMale=Math.max(0, Math.floor(males.length/minG)-baseMale.length)
      const chosenMale=baseMale.concat(empty.slice(0,needMale)).slice(0,gc)
      for(let r=0;r<minG;r++){ for(const gi of chosenMale){ if(!males.length) break; if(canPut(gi)&&put(gi,males[0])) males.shift() } }
      outerM: while(males.length){ let moved=false; for(const gi of chosenMale){ if(!males.length) break outerM; if(canPut(gi)&&put(gi,males[0])){ males.shift(); moved=true } } if(!moved) break }
      const used=new Set(chosenMale)
      const femaleCands=female.concat(empty.filter(i=>!used.has(i)))
      for(let r=0;r<minG;r++){ for(const gi of femaleCands){ if(!females.length) break; if(canPut(gi)&&put(gi,females[0])) females.shift() } }
      outerF: while(females.length){ let moved=false; for(const gi of femaleCands){ if(!females.length) break outerF; if(canPut(gi)&&put(gi,females[0])){ females.shift(); moved=true } } if(!moved) break }
      const fill=(q:Student[])=>{ for(const gi of order){ while(q.length&&canPut(gi)){ if(put(gi,q[0])) q.shift(); else break } } }
      fill(males); fill(females); fill(others)
    } else if(mode==='성비균형'){
      const totalPool=males.length+females.length+others.length
      const maleRatio= totalPool ? males.length/totalPool : .5
      ...
      const cnt=nextGroups.map(g=>({ m:g.students.filter(s=>s.gender==='남').length, f:g.students.filter(s=>s.gender==='여').length, o:g.students.filter(s=>s.gender==='미정').length }))
      for(const gi of order){ while(canPut(gi)&&males.length&&cnt[gi].m<maleTargets[gi]){ if(put(gi,males[0])){ males.shift(); cnt[gi].m++ } else break } }
      for(const gi of order){ while(canPut(gi)&&(females.length||others.length)){ const st=females.length?females[0]:others[0]; if(put(gi,st)){ if(st.gender==='여') females.shift(); else others.shift() } else break } }
      const rest=[...males,...females,...others]
      for(const gi of order){ while(rest.length&&canPut(gi)){ const st=rest[0]; if(put(gi,st)) rest.shift(); else break } }
    } else {
      let all=shuffleArray([...males,...females,...others])
      for(const gi of order){ while(canPut(gi)&&all.length){ const st=all[0]; if(put(gi,st)) all.shift(); else break } }
      let again=true
      while(again){ again=false; for(const gi of order){ if(!all.length) break; if(canPut(gi)){ const st=all[0]; if(put(gi,st)){ all.shift(); again=true } } } }
    }
