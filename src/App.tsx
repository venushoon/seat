...
export default function App() {
  ...
  const [mixGender, setMixGender] = useState<boolean>(true) // OFF면 항상 남여섞기OFF로 처리
  ...
  useEffect(() => {
    const onPrompt = (e: any) => { e.preventDefault(); setDeferredPrompt(e); setInstallAvailable(true) }
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setMixGender(p.mixGender ?? true); setFriendPairs(p.friendPairs || [])
        setAntiPairs(p.antiPairs || [])
      } catch {}
    }
  }, [])
  useEffect(() => {
    const payload = { students, groups, groupCount, minPerGroup, maxPerGroup, mode, mixGender, friendPairs, antiPairs }
  ...
  const effectiveMode: Mode = !mixGender ? '남여섞기OFF' : mode
  ...
  const onDropToGroup = (gidx: number) => (e: React.DragEvent) => {
    ...
    if (!id) return
    const st = students.find(s => s.id === id) || groups.flatMap(g => g.students).find(s => s.id === id)
    if (!st) return
    // 섞지 않기 모드: 그룹 내 성별 검사
    if (effectiveMode === '남여섞기OFF') {
      const g = groups[gidx]
      const genders = new Set(g.students.map(s => s.gender))
      if (st.gender === '미정') return // 수동 배치도 미정은 금지
      if (genders.size > 0 && (!genders.has(st.gender) || genders.has(st.gender === '남' ? '여' : '남'))) return
  ...
  const canAddTo = (g: Group, st: Student): boolean => {
    ...
    if (effectiveMode !== '남여섞기OFF') return true
  ...
  function arrange() {
    ...
    if (effectiveMode === '남여섞기OFF') {
      // 규칙: 미정은 자동 배치 X, 각 모둠은 단일 성별만
      const groupGenderLock: (Gender | 'unset' | 'blocked')[] = nextGroups.map(g => {
        const gg = groupGenderOf(g)
        if (gg === '혼합') return 'blocked' // 잠금 충돌 → 더 이상 추가 불가
        if (gg === '비어있음') return 'unset'
        return gg
      })

      const fill = (queue: Student[], gender: Gender) => {
        for (let idx of cycle(order)) {
          if (!queue.length) break
          // 타깃/상한/성별 검사
          if (!canPut(idx)) continue
          if (groupGenderLock[idx] === 'blocked') continue
          if (groupGenderLock[idx] === 'unset' || groupGenderLock[idx] === gender) {
            const st = queue[0]
            if (put(idx, st)) {
              queue.shift()
              if (groupGenderLock[idx] === 'unset') groupGenderLock[idx] = gender
          ...
          // 종료 조건
          const totalLeft = queue.length
          const anySpace = nextGroups.some((g, gi) =>
            nextGroups[gi].students.length < targets[gi] &&
            (groupGenderLock[gi] === 'unset' || groupGenderLock[gi] === gender)
          )
          if (!anySpace) break
        }
      }
      // 더 많은 성별부터 채우기 → 빈 그룹의 성별이 쉽게 결정되도록
      if (males.length >= females.length) { fill(males, '남'); fill(females, '여') }
      else { fill(females, '여'); fill(males, '남') }
      // others(미정) 자동 배치 안 함 → 미배치 유지
    } else if (mode === '성비균형') {
    ...
    // 4) 제약 만족 (친구 → 떼기, 몇 차례 시도). 실패 시 미배치 유지.
    const canStayWithGenderRule = (g: Group) => effectiveMode !== '남여섞기OFF' || (groupGenderOf(g) !== '혼합')
    const MAX_ITERS = 3
    for (let t = 0; t < MAX_ITERS; t++) {
      ...
      for (const p of friendPairs) {
        ...
        if (canAddTo(nextGroups[bi], nextGroups[ai].students.find(s => s.id === p.aId)!)) { tryMove(nextGroups, p.aId, bi) }
        else if (canAddTo(nextGroups[ai], nextGroups[bi].students.find(s => s.id === p.bId)!)) { tryMove(nextGroups, p.bId, ai) }
        else {
          // 스왑 후보
      ...
      for (const p of antiPairs) {
        ...
        for (const ti of toOrder) {
          if (tryMove(nextGroups, p.bId, ti) && canStayWithGenderRule(nextGroups[ti])) { done = true; break }
        }
        if (!done) {
          for (const ti of toOrder) {
  ...
  function saveAs() {
    ...
    saves[name] = { students, groups, groupCount, minPerGroup, maxPerGroup, mode, mixGender, friendPairs, antiPairs, savedAt: new Date().toISOString() }
  ...
  function loadFrom() {
    ...
    setMode(s.mode || '성비균형'); setMixGender(s.mixGender ?? true)
    setFriendPairs(s.friendPairs || []); setAntiPairs(s.antiPairs || [])
  }
  function exportCSV() {
    const rows: string[] = []
  ...
  function exportJSON() {
    const data = { students, groups, groupCount, minPerGroup, maxPerGroup, mode, mixGender, friendPairs, antiPairs }
  ...
  function importJSON(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        setMode(obj.mode || '성비균형'); setMixGender(obj.mixGender ?? true)
        setFriendPairs(obj.friendPairs || []); setAntiPairs(obj.antiPairs || [])
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {installAvailable && (<button onClick={onInstall} className="btn-primary">PWA 설치</button>)}
          </div>
        </div>
      </header>
            ...
            <div className="sticky top-[72px] space-y-4">
              ...
              <section className="card p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="badge">{effectiveMode === '남여섞기OFF' ? '남/여 분리' : '혼합 허용'}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <label className="flex items-center gap-2 col-span-2 text-[0.95rem]">
                    ...
                    <span>남녀 함께 조합 허용 (OFF 시 강제 분리)</span>
                  </label>
                </div>
